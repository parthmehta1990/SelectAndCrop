...
class CropImageTask extends AsyncTask<Void, Void, Throwable> {
    ...
    protected Throwable doInBackground(Void... params) {
        try {
            ...
            Log.d("CropImageTask=>","=>"+srcUri+"=>"+ saveConfig.getWidth()+"=>"+
                    saveConfig.getHeight()+"=>"+saveConfig.getQuality());
            ...
            Log.d("CropImageTask=>","=>"+cropped.getDensity()+"=>"+cropped.getWidth()+"=>"+cropped.getHeight());

            cropped = mask.applyMaskTo(cropped);

            cropped.setDensity(DisplayMetrics.DENSITY_XXXHIGH);
            Log.d("CropImageTask=>","=>"+cropped.getDensity());
